...
function setupWalletConnect() {
  ...
  btn.addEventListener("click", () => {
    if (!state.walletConnected) {
      ...
      btn.classList.remove("ch-btn-outline");
      ...
      btn.classList.add("ch-btn-outline");
  ...
  function recalc() {
    ...
    if (value <= 0) {
      ...
      priceInfo.textContent =
        "Live estimate based on the Coffee DEX (demo calculation).";
      return;
    }
    ...
    priceInfo.textContent = `Estimated rate ~ 1 LADY ≈ ${implied} BEANS (demo only, in production this comes from the DEX router).`;
  }
...
function updateBrewAccumulated() {
  ...
  // Demo: fill level of coffee in cup up to 80%
  const maxHeight = 80;
  const ratio = Math.min(state.brew.accumulated / 100, 1); // scale
